...
struct Simulation{
    int next_block;
    bool stage; // false -> creation of the copy, true -> simulation
	Block** map;
    Block** copymap;
    mutex mtx;
	int gx; // x-coordinate of ground zero
	int gy; // y-coordinate of ground zero
    Simulation(){gx=0;gy=0;map = NULL;copymap = NULL;next_block = 0;stage = false;};
};
...
void* simulate(void* data){
	...
	while(1){
		sim->mtx.lock();
		if(sim->stage || sim->next_block >= (sizes)){
            sim->stage = true;
			sim->mtx.unlock();
			break;
		}
        ...
        int row,col;
        //read the map per row and copy it per column
		for(int i = 0; i < sizes; i++){
			row = i;
			col = (selected_ground_zero);
            sim->copymap[col][row].solidity = sim->map[row][col].solidity;
		}
    }

    selected_ground_zero = 0;
	sim-> next_block = 0;
    ...
    while(1){
		...
		for(int i = 0; i < sizes; i++){
			...
			sim->map[col][row].devastation = destr_eval(
					sim->copymap[row][col].solidity,
					row,
					col,
					0,
					0
...
int main(int argc, char *argv[]){
	...
	sim.copymap = new Block*[sizes];
	for(int i = 0; i < sizes; i++){
		sim.map[i] = new Block[sizes];
        sim.copymap[i] = new Block[sizes];
    }
	int n = 0;
