...
__global__ void DES(Block* map, int blocchi, int thread) {
    ...
    // gli elementi andranno da id_x * slice ad id_*slice+slice-1;
		...
		for(int i = 0 ; i<slice;i++){
            ...
            if (row < sizes && col < sizes) {
                __syncthreads();
        		float xx = pow((row - gx), 2);
        		float yy = pow((col - gy), 2);

        		float sum_xx_yy = xx + yy;
        		int distance_evaluation = floor(sqrt(sum_xx_yy));
